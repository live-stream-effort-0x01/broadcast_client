...
const SignUpForm: Component<SignUpFormProps> = (props) => {
  ...
  const handlePasswordChange = async (e: Event, value: any) => {
    ...
    if (!isValid) {
      setPasswordError(
        `Your password must include: 6 to 12 characters, 1 lowercase letter, 1 uppercase letter, 1 digit, and 1 special character.`
      );
    } else {
    ...
    <div class='form-wapper'>
      ...
      <form class="form-post" id="register" onSubmit={register}>
        <div class="form-main">
          ...
          {passwordError() && <p class={`form-error-message ${passwordError() ? 'active' : ''}`}>{passwordError()}</p>}
        ...
        <div class="form-btn">
          ...
          <div class="form-more">
            <p class="form-more-content">Already an member?</p>
